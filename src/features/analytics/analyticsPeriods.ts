...
import { formatDateKey, getWeekKey, parseDateKey } from '@/lib/storage'
import type { AnalyticsDateRange, AnalyticsPeriod } from '@/types/analytics'
import type { DailySnapshot } from '@/types/history'
import type { QuestDefinition } from '@/types/quest'

/**
 * Resolves an inclusive quest-day key range for a period, relative to
...
export function resolvePeriodRange(
  ...
  switch (period) {
    ...
    case 'week': {
      ...
      // Walk ±7 days around today — enough to cover any ISO week containing today.
      const center = parseDateKey(todayKey)
      let start = todayKey
      let end = todayKey
      for (let offset = -7; offset <= 7; offset += 1) {
        const cursor = new Date(center)
        cursor.setDate(center.getDate() + offset)
        if (getWeekKey(cursor) !== week) continue
    ...
    case 'month': {
      ...
      const start = formatDateKey(new Date(year, month, 1, 12, 0, 0))
      const end = formatDateKey(new Date(year, month + 1, 0, 12, 0, 0))
      return { start, end }
    }
    case 'lifetime':
      return null
