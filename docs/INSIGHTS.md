...
## Nutrition Insights (v0.0.4)

| Type | Meaning |
|------|---------|
| `nutritionProteinStreak` | Consecutive days the protein target has been met |
| `nutritionMealConsistency` | Streak of days with breakfast + lunch + dinner all logged, or a low overall rate |
| `nutritionMissedMeal` | A required meal type missed on ≥50% of tracked days |
| `nutritionMealTiming` | A meal's average logged time is past a fixed threshold (e.g. "dinner is frequently delayed") |

Generated by `generateNutritionInsights()` (`insightsNutrition.ts`) from `getNutritionAnalytics()` — no new History storage, same "interpret, don't invent" rule as every other insight. See [NUTRITION.md](NUTRITION.md).
...
| **Nutrition analysis** | Implemented (v0.0.4) — see Nutrition Insights above and [NUTRITION.md](NUTRITION.md) |
